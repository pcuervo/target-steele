...
class Zo_Recent_Posts_Widget_With_Thumbnails extends WP_Widget {
	...
	function __construct() {
        ...
        parent::__construct(
            ...
            esc_html__('ZO Recent Posts With Thumbnails', 'lucian'), // Name
            array('description' => esc_html__('List of your site&#8217;s most recent posts, with clickable title and thumbnails.', 'lucian'),) // Args
        );
	...
	function widget( $args, $instance ) {
		...
		<?php while ( $r->have_posts() ) : $r->the_post(); ?>
			<div class="latest-post-item">
					...
					if ( $show_thumb ) : 
						...
						if ( $only_1st_img ) :
							...
							if ( has_post_thumbnail() ) : 
								...
								echo zo_post_thumbnail($thumb_width, $thumb_height, true);
								$is_thumb = true;
							else :
								// else 
				...
				<?php if ( ! $hide_title ) { ?>
					<h3 class="zo-post-title"><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></h3>
				<?php }?>

				<?php if ( $show_date ) : ?>
					<div class="zo-post-date">
						<span><?php the_time('M d, Y'); ?> / </span>
						<span class="zo-blog-comment">
							<a href="<?php the_permalink(); ?>"><?php echo comments_number('0','1','%'); ?></a>
							<?php esc_html_e('comments', 'lucian'); ?>
						</span>
					</div>
...
<?php
	...
	function form( $instance ) {
		...
		?>
		<p><label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Title:', 'lucian' ); ?></label>
		<input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" type="text" value="<?php echo esc_attr($title); ?>" /></p>

		<p><label for="<?php echo esc_attr($this->get_field_id( 'number_posts' )); ?>"><?php esc_html_e( 'Number of posts to show:', 'lucian' ); ?></label>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'show_date' )); ?>"><?php esc_html_e( 'Display post date?', 'lucian' ); ?></label></p>

		<h4><?php esc_html_e( 'Thumbnail Options', 'lucian' ); ?>:</h4>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'show_thumb' )); ?>"><?php esc_html_e( 'Display post featured image?', 'lucian' ); ?></label></p>
		
		<p><label for="<?php echo esc_attr($this->get_field_id( 'thumb_width' )); ?>"><?php esc_html_e( 'Width of thumbnail', 'lucian' ); ?>:</label>
		...
		<p><label for="<?php echo esc_attr($this->get_field_id( 'thumb_height' )); ?>"><?php esc_html_e( 'Height of thumbnail', 'lucian' ); ?>:</label>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'keep_aspect_ratio' )); ?>"><?php esc_html_e( 'Use aspect ratios of original images?', 'lucian' ); ?> <em><?php esc_html_e( 'If checked the given width is used to determine the height of the thumbnail automatically. This option also supports responsive web design.', 'lucian' ); ?></em></label></p>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'hide_title' )); ?>"><?php esc_html_e( 'Do not show title?', 'lucian' ); ?> <em><?php esc_html_e( 'Make sure you set a default thumbnail for posts without a thumbnail, otherwise there will be no link.', 'lucian' ); ?></em></label> </p>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'try_1st_img' )); ?>"><?php esc_html_e( "Try to use the post's first image if post has no featured image?", 'lucian' ); ?></label></p>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'only_1st_img' )); ?>"><?php esc_html_e( 'Use first image only, ignore featured image?', 'lucian' ); ?></label></p>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'use_default' )); ?>"><?php esc_html_e( 'Use default thumbnail if no image could be determined?', 'lucian' ); ?></label></p>
		
		<p><label for="<?php echo esc_attr($this->get_field_id( 'default_url' )); ?>"><?php esc_html_e( 'URL of default thumbnail (start with http://)', 'lucian' ); ?>:</label>

...
<div class="zo-grid-wrapper <?php echo esc_attr( $atts['template'] ); ?>" id="<?php echo esc_attr( $atts['html_id'] ); ?>">
	...
	<div class="row <?php echo esc_attr( $atts['grid_class'] ); ?>">
		...
		while ( $posts->have_posts() ) :
			...
			if ( $i ++ == 0 ) :
				?>
				<div class="zo-blog-large col-lg-6 col-md-6 col-sm-12 col-xs-12"
					...
					<div class="zo-blog-item">
						...
						<<?php echo esc_attr($zo_title_size); ?> class="zo-blog-title">
							<a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>"><?php the_title(); ?></a>
						</<?php echo esc_attr($zo_title_size); ?>>
						<div class="zo-blog-content"><?php echo zo_limit_words( get_the_excerpt(), 20 ); ?></div>
						<div class="zo-blog-more">
							<ul class="zo-blog-meta">
								<li class="zo-blog-date"><?php the_time( 'M d, Y' ); ?></li>
								<li class="zo-blog-comment">
									<a href="<?php comments_link(); ?>"><?php comments_number( '0', '1', '%' ); ?></a> <?php esc_html_e( 'comments', 'lucian') ?>
								</li>
								<li class="zo-blog-author">
									<span class="author vcard"> - <?php esc_html_e('By', 'lucian'); ?> <?php the_author_posts_link(); ?></span>
								</li>
							</ul>
							<a class="zo-blog-readmore btn btn-dark" href="<?php the_permalink(); ?>"><?php esc_html_e('READ MORE', 'lucian'); ?></a>
						</div>
					</div>
				</div>
			<?php endif; ?>
		<?php endwhile; ?>
		<div class="zo-blog-side row col-lg-6 col-md-6 col-sm-12 col-xs-12">
			<?php $j = 0; ?>
			<?php while ( $posts->have_posts() ) :
				...
				?>
				<?php if ( $j ++ != 0 ) : ?>
				<div class="zo-blog-item <?php echo esc_attr( $atts['item_class'] ); ?>"
				     data-groups='[<?php echo implode( ',', $groups ); ?>]'>
					<div class="zo-blog-header">
						<?php
						if ( has_post_thumbnail() ) :
							echo zo_post_thumbnail(170, 170, true);
						endif;
						?>
					</div>
					<div class="zo-blog-body">
						<<?php echo esc_attr($zo_title_size); ?> class="zo-blog-title">
							<a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>"><?php the_title(); ?></a>
						</<?php echo esc_attr($zo_title_size); ?>>
						<div class="zo-blog-content"><?php echo zo_limit_words( get_the_excerpt(), 20 ); ?></div>
